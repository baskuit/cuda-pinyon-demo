...
Wrapper for pkmn/engine battles

The wrapper class is given the type-identifier `State` inside its type list ("BattleTypes")
...
{
    // the base class here is a template using the `TypeList`
    // that adds Pinyon interface data members/methods
    // 'is_terminal`, `terminal`, `apply_actions` etc
